...
# shellcheck source=download-gov.sh
. "$script_dir/download-gov.sh"
unset PRAWKO_GOV_LIBRARY_ONLY
gov_raw="$(gov_data_dir)/raw"
server=""
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    --help|-h|-Help)
      ...
      echo "  --source DIR   raw JPG/WMV (default: gov-data/raw)"
...
media_fallback="$(dirname "$(gov_data_dir)")/media"
if [ -z "$IMG_OUT" ]; then
...
resolve_ffmpeg() {
  if [ -n "$FFMPEG_EXE" ]; then
    [ -x "$FFMPEG_EXE" ] || die "ffmpeg not found: $FFMPEG_EXE"
    printf '%s\n' "$FFMPEG_EXE"
    return
  fi
...
[ -d "$SOURCE_DIR" ] || die "Source directory not found: $SOURCE_DIR (run scripts/download-gov.sh first)."

ffmpeg="$(resolve_ffmpeg)" || die "FFmpeg is not available. Install ffmpeg (brew / apt / dnf) or run the installer with --install-gov."
say_g "[OK] FFmpeg: $ffmpeg"
say_d "Source (situational): $SOURCE_DIR"
say_d "Output: $IMG_OUT  /  $VID_OUT"
say_d "gov-data/pjm is not converted."
...
say_c "-> Converting images JPG → WebP ($count files)..."
i=0
for img in "$SOURCE_DIR"/*.jpg "$SOURCE_DIR"/*.JPG "$SOURCE_DIR"/*.jpeg "$SOURCE_DIR"/*.JPEG; do
  [ -f "$img" ] || continue
  i=$((i + 1))
...
say_c "-> Converting videos WMV → MP4 ($count files, this may take a while)..."
i=0
for vid in "$SOURCE_DIR"/*.wmv "$SOURCE_DIR"/*.WMV; do
  [ -f "$vid" ] || continue
  is_pjm_name "$vid" && continue
  ...
  if [ "$videotoolbox" -eq 1 ]; then
    if ! "$ffmpeg" -y -hide_banner -loglevel error -i "$vid" \
      ...
      say_y "   skipped (ffmpeg error): $(basename "$vid")"
    fi
  else
    if ! "$ffmpeg" -y -hide_banner -loglevel error -i "$vid" \
...
say_g "-> App media ready: $webp_n WebP, $mp4_n MP4"
